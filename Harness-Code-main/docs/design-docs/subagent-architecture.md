...
- `purpose="long_term_memory_extraction"`：仅暴露 `read_file`/`grep`/`glob`/`write_file`/`edit_file`，state 写 `long_term_memory_extraction_agent=True` 和 `allowed_memory_dir`，写入限于 `.harness-code/memory/` 下 `.md`。
