...
内部消息角色：`user`、`assistant`、`tool_result`、`attachment`。Harness-code 内部保留 provider-neutral `tool_result`，provider adapter 投影为目标 wire format；`attachment` 是 durable internal role，由 context preparer 在调用前投影后隐藏（详见 `attachment-architecture.md`）。`current_messages()` 返回 deepcopy，避免外部直接修改内部状态。
...
flowchart TD
  ...
  Tr --> Jsonl[".harness-code/sessions/&lt;session&gt;/messages.jsonl"]
  Tr -->|tool_result content > 50KB| Ext[".harness-code/sessions/&lt;session&gt;/tool-results/&lt;id&gt;.txt"]
...
- 消息：`.harness-code/sessions/<session_id>/messages.jsonl`
- 外置 tool result：`.harness-code/sessions/<session_id>/tool-results/<id>.txt`
