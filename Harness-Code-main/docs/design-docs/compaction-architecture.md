...
`SessionMemory` 字段：`content`、`last_summarized_message_uuid`、`updated_at`、`covered_turn_count`、`source`。`SessionMemoryStore` 读写 `.harness-code/sessions/<session_id>/session-memory.md`，只服务当前会话压缩后的连续性，不是跨会话长期记忆。
...
- session memory：`.harness-code/sessions/<session_id>/session-memory.md`
- shared tool result storage：`.harness-code/sessions/<session_id>/tool-results/<result_id>.txt`
